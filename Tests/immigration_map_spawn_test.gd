...
func _run() -> void:
	...
	for index: int in range(migrants.size()):
		...
		if is_instance_valid(migrant) and migrant.global_position.distance_to(initial_positions[index]) > 0.1:
			moving_count += 1
	_expect(moving_count == 2, "有移民出生后没有沿可达路径移动")
	print("移民地图出生测试", "失败" if _failed else "通过", "：人数=", migrants.size())
